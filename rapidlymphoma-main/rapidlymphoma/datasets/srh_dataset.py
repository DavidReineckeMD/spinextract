"""PyTorch datasets designed to work with rapidlymphoma.
...
from rapidlymphoma.datasets.improc import process_read_im, get_srh_base_aug
...
class SRHBaseDataset(Dataset):
    """Abstract base class for rapidlymphoma dataset.

    Args:
        data_root: root rapidlymphoma directory
    ...
    def get_all_meta(self):
        ...
        try:
            with open(os.path.join(self.data_root_,
                                   "meta/rapidlymphoma.json")) as fd:
                self.metadata_ = json.load(fd)
        except Exception as e:
            logging.critical("Failed to locate dataset.")
            raise e

        logging.info(f"Locate rapidlymphoma dataset at {self.data_root_}")
        return
...
class SRHClassificationDataset(SRHBaseDataset):
    """rapidlymphoma Patch dataset for classification (cross entropy)
...
class SRHContrastiveDataset(SRHBaseDataset):
    ...
    Can be used for BYOL experiments
