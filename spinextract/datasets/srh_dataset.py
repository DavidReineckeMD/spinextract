"""PyTorch datasets designed to work with spinextract.
...
from spinextract.datasets.improc import process_read_im, get_srh_base_aug
...
class SRHBaseDataset(Dataset):
    """Abstract base class for spinextract dataset.

    Args:
        data_root: root spinextract directory
    ...
    def get_all_meta(self):
        ...
        try:
            with open(os.path.join(self.data_root_,
                                   "meta/spinextract.json")) as fd:
                self.metadata_ = json.load(fd)
        except Exception as e:
            logging.critical("Failed to locate dataset.")
            raise e

        logging.info(f"Locate spinextract dataset at {self.data_root_}")
        return
...
class SRHClassificationDataset(SRHBaseDataset):
    """spinextract Patch dataset for classification (cross entropy)
