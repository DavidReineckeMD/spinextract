...
CONF_HIGH_THRESHOLD = 0.77 # based empirically on multicenter testcohort of SpineXtract paper (see paper)
...
def make_specs(predictions: Dict[str, Union[torch.Tensor, List[str]]]) -> None:
    ...
    if "probs" in predictions:
        probs_np = predictions["probs"].detach().cpu().numpy() if isinstance(predictions["probs"], torch.Tensor) else np.asarray(predictions["probs"])
        pred["logits"] = [p for p in probs_np]
    else:
        pred["logits"] = [torch.softmax(t, dim=0) for t in list(predictions["logits"])]
...
def load_calibration(prefix: str):
    calib = joblib.load(f"{prefix}_platt_calib.joblib")
    with open(f"{prefix}_youden.json", "r") as f:
        thresholds = json.load(f)
    return calib, thresholds


def apply_platt_cv_to_probs(probs: np.ndarray, calib: Dict[str, Any]) -> np.ndarray:
    n_classes = len(calib["class_names"])
    preds_per_fold = []
    for fold_clfs in calib["clfs_per_fold"]:
        Pk = np.zeros_like(probs, dtype=float)
        for i in range(n_classes):
            Pk[:, i] = fold_clfs[i].predict_proba(probs[:, i].reshape(-1, 1))[:, 1]
        preds_per_fold.append(Pk)
    P_cal = np.mean(preds_per_fold, axis=0)
    P_cal = np.clip(P_cal, 1e-8, 1.0)
...
def save_pred_csvs(predictions: Dict[str, Union[torch.Tensor, List[str]]], out_dir: str,
    ...
    if "probs" in predictions:
        if isinstance(predictions["probs"], torch.Tensor):
            probs = predictions["probs"].detach().cpu().numpy()
        else:
            probs = np.asarray(predictions["probs"], dtype=np.float32)
    else:
    ...
    def _agg(df: pd.DataFrame, by: str, out_name: str):
        g = df.groupby(by)
        rows = []
        for key, grp in g:
            ...
            row = {
                ...
                "high_confidence": bool(conf >= conf_high_threshold),
            }
            for i in range(n_classes):
    ...
    slide_df = _agg(patch_df, "slide", "predictions_slide.csv")
    patient_df = _agg(patch_df, "patient", "predictions_patient.csv")
...
def main():
    ...
    if cf["eval"].get("platt_prefix", None):
        calib, thresholds = load_calibration(cf["eval"]["platt_prefix"])
        logits = predictions["logits"]
        if isinstance(logits, torch.Tensor):
            base_probs = torch.softmax(logits, dim=1).cpu().numpy()
        else:
            base_probs = _to_probs(logits)
        probs_cal = apply_platt_cv_to_probs(base_probs, calib)
        predictions["probs"] = torch.from_numpy(probs_cal).float()
    else:
        logits = predictions["logits"]
        if isinstance(logits, torch.Tensor):
            predictions["probs"] = torch.softmax(logits, dim=1)
        else:
            predictions["probs"] = torch.from_numpy(_to_probs(logits)).float()
