"""Common modules for spinextract training and evaluation.
...
from spinextract.models import Classifier, TransformerHead, MLP, resnet_backbone, vit_backbone
from spinextract.datasets.srh_dataset import (SRHClassificationDataset,
                                          SRHContrastiveDataset)
from spinextract.datasets.improc import get_srh_aug_list
...
class CEBaseSystem(pl.LightningModule):
    ...
    def __init__(self, cf: Dict[str, Any], nc: int, num_it_per_ep: int):
        ...
        if cf["training"].get("finetune", {}).get("mode") == "transformer":
            bb = get_backbone(cf)
            head = partial(TransformerHead,
                        n_in=bb().num_out,
                        hidden_layers=cf["model"].get("mlp_hidden", []),
                        n_out=nc,
                        num_heads=cf["model"].get("num_heads", 8),
                        num_layers=cf["model"].get("num_layers", 3),
                        dropout=cf["model"].get("dropout", 0.3))
            self.model = Classifier(bb, transformer)
        else:
            bb = get_backbone(cf)
            head = partial(MLP,
                        n_in=bb().num_out,
