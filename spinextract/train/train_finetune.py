"""Finetuning experiment with transformer-based classifier training script.
...
from spinextract.train.common import (setup_output_dirs, parse_args, get_exp_name,
                                  get_dataloaders, config_loggers,
                                  get_optimizer_func, get_scheduler_func,
                                  CEBaseSystem)


class FinetuneSystem(CEBaseSystem):
    """Lightning system for finetuning contrastive experiments."""

    def __init__(self, cf: Dict[str, Any], nc: int, num_it_per_ep: int):
        super().__init__(cf, nc, num_it_per_ep)

        # freeze the weights of backbone for linear evaludation
        self.freeze_weights_ = False
        if cf["training"]["finetune_mode"] == "linear":
            self.model.bb.requires_grad = False
            self.freeze_weights_ = True

    def training_step(self, batch, batch_idx):
        if self.freeze_weights_:
            self.model.bb.eval()
    ...
    def configure_optimizers(self):
        # get params
        if self.cf_["training"]["finetune_mode"] in ["linear", "transformer"]:
            for p in self.model.bb.parameters():
                p.requires_grad = False
            params = self.model.head.parameters()
            logging.info("linear")
        elif self.cf_["training"]["finetune_mode"] == "finetune":
        ...
        # get optimizer
        opt = get_optimizer_func(self.cf_)(params)
...
def main():
    ...
    ce_exp = FinetuneSystem(cf, nc, num_it_per_ep)

    # load lightning checkpint
    assert "backbone_checkpoint" in cf["training"]
